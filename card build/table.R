...
getmedian <- function(finframe){
  ...
  if(length(notype) == length(names)){       #falls keine Types definiert wurden gib den frame zurück
    ...
    meddesease<- rowSums(finframe[,startdesease:enddesease])/length(desease)
    medcontrol<- rowSums(finframe[,startcontrol:ncol(finframe)])/length(control)
    
    print(startdesease)
    print(enddesease)
    print(startcontrol)
    print(ncol(finframe))
    finframe<-cbind(finframe,meddesease,medcontrol)
  }
